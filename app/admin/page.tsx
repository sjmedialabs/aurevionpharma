...
import { Package, FolderTree, Mail, TrendingUp, AlertCircle, CheckCircle2, Clock } from "lucide-react"
...
export default async function AdminDashboard() {
  ...
  return (
    <>
      <PageHeader
        title="Dashboard"
        description="Welcome to Aurevion Admin CMS"
      />
      <div className="p-8">
      ...
      <div className="mb-8">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Enquiry Status Overview</h2>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="dark:bg-gray-800 dark:border-gray-700">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="dark:text-white">Recent Enquiries</CardTitle>
            <Link 
              href="/admin/enquiries" 
            ...
            <div className="space-y-4">
              {enquiries.length > 0 ? (
                enquiries.slice(0, 5).map((enquiry) => (
                  <div key={enquiry.id} className="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-3 last:border-0">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-gray-900 dark:text-white truncate">{enquiry.name}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400 truncate">{enquiry.email}</p>
                      <div className="flex items-center gap-2 mt-1">
                        <p className="text-xs text-gray-500 dark:text-gray-400">{enquiry.type || 'general'}</p>
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      <span
                        className={cn(
                          ...
                          enquiry.status === "pending" && "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300",
                          enquiry.status === "contacted" && "bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300",
                          enquiry.status === "resolved" && "bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300",
                        )}
                      >
                        {enquiry.status}
                      </span>
            ...
            <div className="space-y-4">
              {productsCount > 0 ? (
                products.slice(0, 5).map((product) => (
                  <div key={product.id} className="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-3 last:border-0">
                    <div className="min-w-0 flex-1">
                      <p className="font-medium text-gray-900 dark:text-white truncate">{product.name}</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400">CAS: {product.casNumber}</p>
                    ...
                    <div className="ml-4">
                      <span
                        className={cn(
                          "rounded-full px-2 py-1 text-xs font-medium whitespace-nowrap",
                          product.inStock ? "bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300" : "bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300",
                        )}
                      >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="font-medium text-yellow-800 dark:text-yellow-200">
                    You have {pendingEnquiries} pending enquir{pendingEnquiries === 1 ? 'y' : 'ies'}
                  </p>
