...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://aurevion-cluster.plvbx7e.mongodb.net/aurevion';
...
async function addIndexes() {
  try {
    ...
    await productsCollection.createIndex({ casNumber: 1 }, { background: true });
    console.log('✓ Created index on casNumber');
    
    // Compound index for category + inStock (for filtered queries)
    await productsCollection.createIndex({ category: 1, inStock: 1 }, { background: true });
    console.log('✓ Created compound index on category + inStock');
    
    // Index for sorting by createdAt
    await productsCollection.createIndex({ createdAt: -1 }, { background: true });
